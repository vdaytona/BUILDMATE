...
| # | Task | Detail | Status |
|---|------|--------|--------|
| 1.11.1 | Normalise User name fields | Server returns computed `firstName`/`lastName` in auth responses. Removed `mapUser()` from frontend `auth.ts`. | ✅ |
| 1.11.2 | Normalise Task progress field | Server task endpoints now return `progress` alias alongside `percentComplete`. Frontend uses `progress` directly. | ✅ |
| 1.11.3 | Normalise auth response field | Server auth endpoints now return `token` instead of `accessToken`. Frontend reads `data.token` directly. | ✅ |
| 1.11.4 | Normalise status enum casing | Server auth endpoints now return lowercase `role`. User role `.toLowerCase()` hack removed from frontend `mapUser()`. | ✅ |
| 1.11.5 | Prisma Decimal→Number conversion | Created shared Prisma client at `server/src/lib/prisma.ts` with `$extends` query extension that converts all `Decimal` objects to plain JavaScript numbers. Prevents string concatenation bugs when summing monetary values. Updated all route files, `index.ts`, and `auditLog.ts` to import from `../lib/prisma`. | ✅ |
...
| 2.2.9 | Retention release tracking | Track retention held per invoice (% and amount). Add `retentionReleaseDate` field. Show retained/released amounts on project financial summary. Allow manual release with date. | ✅ (UI display only — retention release mechanism requires schema migration) |
...
| 3.1.17 | Drag-to-reorder task rows | GripVertical drag handle on each task row (list + Gantt). HTML5 drag-and-drop reorders rows. POST `/tasks/reorder` bulk updates `sortOrder`. Optimistic UI. | ✅ |
| 3.1.18 | Fix Gantt drag-to-reschedule snap-back | Optimistic cache updates in `updateTaskMutation` (`onMutate`) prevent bar snapping back while waiting for server response. | ✅ |
| 3.1.19 | Gantt horizontal panning | Mouse drag on empty timeline space pans left/right. `panState` tracks drag start position. `cursor-grab`/`cursor-grabbing` feedback. | ✅ |
| 3.1.20 | Construction stage column | New `stage` field on Task model (12 values: INITIATION → DEFECT_LIABILITY). Stage dropdown in edit modal. Stage column in task list. `TASK_STAGES` constant on server. | ✅ |
| 3.1.21 | Task reorder API endpoint | POST `/api/projects/:id/tasks/reorder` — accepts `{ projectId, orderedTaskIds }`. Bulk updates `sortOrder` in a Prisma transaction. | ✅ |
| 3.1.22 | Fix Gantt bar drag vs click vs pan | Removed `onClick` from bars entirely. Click detection handled in `mouseup`: local `moved` flag tracks >3px movement — if no movement, calls `onTaskClick` (edit modal). If moved, applies reschedule. No race conditions possible. | ✅ |
| 3.1.23 | Day-of-week labels in Gantt header | Week headers show day labels when `dayWidth >= 28` (default zoom): single letter (M, T, W…) at 28–39px, full name (Mon, Tue…) at 40px+. Hidden when zoomed out below 28px. Header 44px when labels shown. | ✅ |
...
| 5.3.7 | Document API normalisation | Fix category↔folder mapping (lowercase→uppercase), flatten `uploadedBy` object to name string, handle null `mimeType`, fix SQLite search (`mode: 'insensitive'` removed), add `/uploads` Vite proxy, fix TS errors in server routes. | ✅ |
...
| 6.8 | CSRF protection | Add CSRF tokens for state-changing requests if cookie-based auth is added. JWT Bearer token auth (not cookies) makes CSRF inherently mitigated. | ✅ |
...
| Phase 5 — Inspections & Documents | 20 | 14 | 0 | 6 |
| Phase 6 — Security & Access Control | 9 | 6 | 0 | 3 |
| Phase 6B — Non-Functional Requirements | 6 | 0 | 0 | 6 |
| Phase 7 — Production Readiness | 16 | 12 | 0 | 4 |
| Phase 8 — iPad & iPhone App | 54 | 54 | 0 | 0 |
| **Total** | **231** | **105** | **0** | **126** |
